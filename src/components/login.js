...
function Login() {
    ...
    const submitForm = (ev) => {
        ...
        if (customer.email_id === "admin@gmail.com" && customer.password === "admin@123") {
            ...
            fetch("http://localhost:8081/user/CheckLogin", reqOptions)
                .then(resp => resp.text())
                .then((data) => {
                    setMsg(data);
                    if (data != '') {
    ...
    return (
        <center>
            <div className="container-fluid" style={{ backgroundImage: `url(${leg})`, height: '595px', backgroundRepeat: "no-repeat", backgroundSize: 'cover' }}>
