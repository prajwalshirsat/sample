...
export default class Maidbookinginfo extends React.Component {
  ...
  componentDidMount = (maidId, categoryname) => {
    ...
    fetch("http://localhost:8081/categoryId?maidId=" + maidId + "&categoryname=" + categoryname)
  ...
  getMaids = (maidId) => {
    fetch("http://localhost:8081/getmaids?maid_id=" + maidId)
      .then(resp => resp.json())//.then(data => alert(JSON.stringify(data)))
  ...
  render() {
    ...
    return (
      ...
      <div className="container-fluid" style={{ backgroundImage: `url(${qw})`, height: '895px', backgroundRepeat: "no-repeat", backgroundSize: 'cover' }}>
