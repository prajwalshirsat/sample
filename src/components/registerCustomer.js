...
class RegisterCustomer extends React.Component {
  ...
  register = (event) => {
    ...
    if (ValidateForm(this.state.errors)) {
      ...
      fetch('http://localhost:8081/user/userinsert', user)
        .then(res => {
          if (res.status == 200) {
  ...
  render() {
    ...
    return (
      <div className="container-fluid" style={{ backgroundImage: `url(${maid})`, height: '1000px', backgroundPosition: "center", backgroundRepeat: "no-repeat", backgroundSize: 'cover' }}>
