...
export default class Bookmaid extends React.Component {
  ...
  book = (event) => {
    ...
    fetch('http://localhost:8081/BookingInfo/insertbooking/' + maidId + '/' + userId + '/' + servicesId, requestOption)
      .then(res => {
        if (res.status == 200) {
  ...
  render() {
    ...
    return (
      ...
      <div className="container-fluid" style={{ backgroundImage: `url(${reg})`, height: '895px', backgroundRepeat: "no-repeat", backgroundSize: 'cover' }}>
